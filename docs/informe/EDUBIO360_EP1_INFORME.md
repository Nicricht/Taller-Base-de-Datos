...
VARRAY resulta adecuado cuando se conoce de antemano un límite máximo de elementos. En EDUBIO360 se utiliza con una capacidad de cinco posiciones, suficiente para conservar el grupo reducido de alternativas definido por la consulta. Además, `BULK COLLECT` permite cargar varias filas desde SQL hacia PL/SQL en una sola operación, evitando recuperar cada resultado de forma individual. Esto hace más eficiente la transferencia de ese pequeño conjunto de datos antes de recorrerlo con un LOOP. Por esta razón, la elección de VARRAY responde al tamaño controlado del conjunto que se necesita procesar y no a una colección de crecimiento indefinido.

En el caso de RECORD, su aporte a la eficiencia se relaciona principalmente con la organización del procesamiento: los atributos de una misma oferta se manipulan como una sola estructura lógica. RECORD no hace que una consulta sea automáticamente más rápida, pero simplifica el manejo de los datos y reduce la cantidad de variables independientes que deben administrarse dentro del bloque.
